...
class AutoSelling {
  ...
  async runPollingLoop() {
    ...
    // Group items by brand_id
    const groupedByBrand = _.groupBy(items, 'brand_id');
    // console.dir(groupedByBrand, { depth: null });
    // process.exit(1);

    for (const [brandId, brandItems] of Object.entries(groupedByBrand)) {
      let brandStatus = null;
      try {
        brandStatus = await ncncService.getBrandStatus(brandId);
      } catch (e) {
        const requestFail = `니콘내콘 리퀘스트 실패`;
        await messageHandler(requestFail);
        logger.error(requestFail, e);
        continue; // Skip to next brand if this request fails
      }

      // console.dir(brandItems)
      // process.exit(1);

      for (const _item of brandItems) {
        const targetItem = _.find(brandStatus, { id: _item.item_id });
        ...
        const isBlock = _.get(targetItem, "isBlock");
        const isRefuse = _.get(targetItem, "isRefuse");
        if (isBlock === 0 && isRefuse === 0 && _item.price <= price) {
          ncncEvent.emit("purchase", _item, targetItem);
          await autoSelling.delay(2000);
        }
        // process.exit(1);
      }
      await autoSelling.delay(2000);
    ...
    setTimeout(() => ncncEvent.emit("poll"), 2000);
  }
